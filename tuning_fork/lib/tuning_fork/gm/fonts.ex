defmodule TuningFork.Gm.Fonts do
  @moduledoc "The soundfont files Strudel plays each `gm_*` name from, in the order `n` picks them."
  ...
  @fonts %{
    ...
    "gm_fretless_bass" => ["0350_Aspirin_sf2_file", "0350_JCLive_sf2_file"],
    "gm_slap_bass_1" => [
  ...
  @doc "The font files for `name`, or `nil` for a name that has none."
  @spec files(String.t()) :: [String.t()] | nil
  def files(name) when is_binary(name), do: Map.get(@fonts, name)

  @doc "The file `n` picks for `name`, wrapping round the list; `nil` for a name that has none."
  @spec file(String.t(), integer()) :: String.t() | nil
  def file(name, n) when is_binary(name) and is_integer(n) do
    case Map.get(@fonts, name) do
      nil -> nil
      files -> Enum.at(files, Integer.mod(n, length(files)))
    end
  end
end
